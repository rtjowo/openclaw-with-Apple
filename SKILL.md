...
2. **双写规则（核心，严格执行）**：
   - **所有事项 → 必须写待办 JSON**：不管有没有具体时间，全部通过 `tasks_tool.py add` 写入待办
   - **有具体时间点 → 额外写日历**：用户**明确说了几点几分**（如"2点"、"14:00"、"早上8点"）→ **同时**调用 `icloud_calendar.py new` 写入日历
   - **没有具体时间点 → 严禁写日历**：用户只说了"明天"、"下周"、"晚上"等模糊时间 → **只写待办 JSON，绝对不写日历，也绝对不要自己编造一个时间填进日历**
   - ⛔ **严禁行为**：用户没说几点，AI 自己脑补一个时间（如把"明天去洗车"变成"14:00-15:00 洗车"）写进日历。这是严重错误！
   - ✅ 正确示例：
     - 用户说"明天2点开会" → `tasks_tool.py add "开会" --date tomorrow --time 14:00` + `icloud_calendar.py new tomorrow 14:00 15:00 "开会"`（⏰📅）
     - 用户说"明天去洗车" → **只执行** `tasks_tool.py add "洗车" --date tomorrow`，**不写日历**（⏰）
     - 用户说"后天早上8点打太极拳" → 待办 + 日历（⏰📅）
     - 用户说"明天晚上去开会" → **只写待办**，"晚上"不是具体时间点（⏰）
3. **目标判断**：
   - 有明确时间或需要执行的动作 → `--target reminder`（提醒事项）
   - 信息记录、笔记、想法、纪要 → `--target note`（备忘录）
4. **智能解析**：从自然语言中提取日期（"明天"、"下周三"）、时间（"下午2点"）、优先级（"重要"→high）
5. **主动确认**：添加后简短确认，告知用户写了什么、怎么同步：
   - 只写待办："已加到待办文件中，今晚十点自动同步至提醒事项 ✓"
   - 双写（待办+日历）："已加到待办文件和日历中，待办今晚十点同步至提醒事项 ✓📅"
   - **必须如实告知**，不能说加了日历但实际没加，也不能没说几点就偷偷加日历
6. **合并整理**：用户说"帮我看看明天的待办"时，调用 `tasks_tool.py list --date tomorrow`
7. **手动同步**：用户说"现在就推送到手机"时，调用 `tasks_tool.py sync`（不等定时任务）
